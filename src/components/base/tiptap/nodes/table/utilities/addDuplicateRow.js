...
export default function addDuplicateRow(
  ...
  const isRowHeader = rowIsHeader(map, table, row + refRow)
  for (let col = 0, index = map.width * row; col < map.width; col++, index++) {
    ...
    if (
      ...
      const _refRow =
        ...
        refRow === null ||
        !(index + refRow * map.width in map.map)
          ? null
          : table.nodeAt(map.map[index + refRow * map.width])
      const type = isRowHeader
      ...
      const node =
        _refRow !== null
          ? withContent
            ? type?.create(
                isRowHeader ? null : { ..._refRow.attrs },
                _refRow.content
              )
            : type?.createAndFill(isRowHeader ? null : { ..._refRow.attrs })
          : type?.createAndFill()

...
import { deleteTableWhenAllCellsSelected } from './utilities/deleteTableWhenAllCellSecleted'
...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    table: {
      ...
      /**
       * Set a cell selection inside the current table
       * @param cellPosition The first cell position
       * @returns True if the command was successful, otherwise false
       * @example editor.commands.setRowSelection(2)
       */
      setRowSelection: (cellPosition: number) => ReturnType
      /**
       * Set a cell selection inside the current table
       * @param position The cell position
       * @returns True if the command was successful, otherwise false
       * @example editor.commands.setCellSelection({ anchorCell: 1, headCell: 2 })
       */
      setColSelection: (cellPosition: number) => ReturnType
      /**
       * Set a cell attribute
       * @param name The attribute name
       * @param value The attribute value
       ...
       * @returns True if the command was successful, otherwise false
       * @example editor.commands.setAllTableCellAttribute('align', 'right')
       */
      ...
      /**
       * Set a cell attribute
       * @param withContent if true content will be copied (default:true)
       * @returns True if the command was successful, otherwise false
       * @example editor.commands.duplicateRow()
       */
       ...
       * @param withContent if true content will be copied (default:true)
       * @returns True if the command was successful, otherwise false
       * @example editor.commands.duplicateColumn()
       */
...
export const Table = Node.create<TableOptions>({
    ...
    return {
      ...
      setColSelection:
        ...
        ({ tr: transaction, dispatch }) => {
          if (dispatch) {
            const position = transaction.doc.resolve(cell)
            const rowSelection = CellSelection.colSelection(position)
            transaction.setSelection(rowSelection)
          }
          return true
        },
      ...
      duplicateColumn:
        ...
        ({ state, dispatch }) => {
          ...
          if (dispatch) {
            const rect = selectedRect(state)
            dispatch(
              addDuplicateColumn(state.tr, rect, rect.right, withContent)
            )
          }
          return true
        }
  ...
  //         return null
  //         // return state.tr.replaceRangeWith(start, end, createdNode)
  //       }
  //     })
  //   ]
  // },

...
import { IMathPluginState } from './mathPlugin'
...
export class MathView implements NodeView {
  ...
  renderMath() {
    ...
    const formatText = texString.replaceAll('$', '')
    try {
      Katex.render(formatText, this._mathRenderElt, this._katexOptions)
